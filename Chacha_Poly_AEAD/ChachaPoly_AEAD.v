module ChachaPoly_AEAD(
    ...
    always @(*) begin
        ...
        poly_valid = 1'b0; poly_I = 32'd0; poly_type_in = 3'd0; poly_byte_vld = 3'd4; poly_is_last = 1'b0;
        chacha_ready_in = 1'b0; chacha_I = 32'd0; valid_out = 1'b0; O = 32'd0;

        // KHI CHACHA CẦN NẠP STATE MATRIX (Block_0 để tạo Khóa Poly, hoặc Block_1,2... để mã hóa Payload)
        if (main_state == 4'd4 || main_state == 4'd8) begin
            if (cnt < 4) chacha_I = 32'd0; // Lõi ChaCha tự biết 4 Word đầu = 0 thì nạp 4 Hằng số (expand 32-byte k)
            else if (cnt < 12) chacha_I = key_reg[cnt - 4]; // Nạp 8 Words Key
            else if (cnt == 12) chacha_I = {block_counter[7:0], block_counter[15:8], block_counter[23:16], block_counter[31:24]}; // Nạp 1 Word Counter (Đảo Little-Endian)
            else chacha_I = nonce_reg[cnt - 13]; // Nạp 3 Words Nonce
        end
        // KHI TẠO KHÓA POLY (Block_counter = 0)
        else if (main_state == 4'd6) begin
            poly_I = chacha_O; // Lấy 256-bit (8 Words) đầu tiên của ChaCha Keystream làm Khóa Poly (r và s)
            poly_type_in = (cnt < 4) ? 3'd0 : 3'd1; // 4 Word đầu là khóa r (type 0), 4 Word sau là khóa s (type 1)
            poly_byte_vld = 3'd4;
            if (cnt < 8) begin 
                poly_valid = chacha_valid_out; // Bơm vào Poly khi ChaCha nhả Keystream
                chacha_ready_in = poly_ready;  // Báo ChaCha đẩy Word tiếp theo khi Poly báo nhận thành công
            end 
            else chacha_ready_in = 1'b1; // Xả bỏ 8 Words Keystream còn lại (PolyKey chỉ cần 256 bit đầu của khối ChaCha 512 bit)
        end
        // KHI MÃ HÓA/GIẢI MÃ PAYLOAD & BĂM AAD (Block_counter > 0)
        else if (main_state == 4'd9 || main_state == 4'd10) begin
            if (valid) begin
                if (type_in == 3'd2) begin // NẾU LÀ AAD (Additional Authenticated Data)
                    poly_valid = 1'b1; poly_I = clean_I; poly_type_in = 3'd2; poly_byte_vld = byte_vld; poly_is_last = is_last;
                    // AAD không cần đi qua ChaCha, đẩy thẳng vào lõi Poly để băm
                end
                else if (type_in == 3'd3) begin // NẾU LÀ PAYLOAD (Cần mã hóa)
                    chacha_I = I; // Đưa Payload vào ChaCha (XOR ngầm bên trong lõi ChaCha)
                    poly_valid = chacha_valid_out; poly_type_in = 3'd3; poly_byte_vld = byte_vld; poly_is_last = is_last;
                    
                    // CHUẨN AEAD: Luôn luôn băm Ciphertext. 
                    // - Nếu đang Mã hóa: Băm đầu ra (Ciphertext vừa sinh ra). 
                    // - Nếu đang Giải mã: Băm đầu vào (Ciphertext nạp từ ngoài vào).
                    poly_I = encrypt ? clean_O : clean_I; 
                    O = clean_O; // Đẩy dữ liệu đã XOR ra ngoài Top Module
                    
                    valid_out = poly_ready && chacha_valid_out; // Báo Testbench biết O đã hợp lệ
                    chacha_ready_in = poly_ready; // Báo ChaCha đẩy Keystream tiếp theo khi Poly đã nuốt xong Ciphertext
                end
                else if (type_in == 3'd4) begin // NẾU GẶP LỆNH KẾT THÚC
                    poly_valid = 1'b1; poly_type_in = 3'd4; poly_is_last = 1'b1; // Ép lõi Poly chốt số, băm chiều dài và ra Tag
                end
            end
        end
    end
    ...
    always @(posedge clk) begin
        if (start) begin
            ...
            case (main_state)
                ...
                4'd1: begin if (valid && type_in == 3'd0) begin key_reg[cnt[2:0]] <= I; if (cnt == 7) begin main_state <= 4'd2; cnt <= 0; end else cnt <= cnt + 1; end end
                
                // STATE 2: Nạp 3 Words Nonce. Xong thì chốt Counter = 0 để cbi sinh PolyKey
                4'd2: begin if (valid && type_in == 3'd1) begin nonce_reg[cnt[1:0]] <= I; if (cnt == 2) begin main_state <= 4'd3; cnt <= 0; block_counter <= 0; end else cnt <= cnt + 1; end end
                
                // STATE 3: Kích hoạt lõi ChaCha bắt đầu chạy
                4'd3: begin chacha_start <= 1; main_state <= 4'd4; cnt <= 0; end
                
                // STATE 4: Bơm 16 Words cấu hình (State Matrix) vào ChaCha
                4'd4: begin if (cnt < 15) cnt <= cnt + 1; else main_state <= 4'd5; end
                
                // STATE 5: Đợi ChaCha trộn xong 20 Vòng (Rounds)
                4'd5: begin if (chacha_valid_out) begin main_state <= 4'd6; cnt <= 0; end end
                
                // STATE 6: Rút Keystream (Block 0) ra làm Khóa r và s cho lõi PolyMAC. Xong thì tăng Counter lên 1.
                4'd6: begin if (chacha_ready_in && chacha_valid_out) begin if (cnt == 15) begin main_state <= 4'd7; chacha_start <= 0; block_counter <= 1; end else cnt <= cnt + 1; end end
                
                // STATE 7: Kích hoạt ChaCha chạy để chuẩn bị Keystream mã hóa Payload
                4'd7: begin chacha_start <= 1; main_state <= 4'd8; cnt <= 0; end
                
                // STATE 8: Bơm 16 Words cấu hình (Block 1, 2...) vào ChaCha
                4'd8: begin if (cnt < 15) cnt <= cnt + 1; else begin main_state <= 4'd9; cnt <= 0; end end
